...
@section('content')
    @php
        ...
        $gift = $product->gift_name ?? null; // Check for the gift name
        $giftImage = $product->gift_image ?? null; // Check for the gift image
    @endphp

    <style>
        /* Styling for the Gift Badge */
        .gift-badge {
            position: absolute;
            top: 10px;
            left: 10px;
        ...
        /* Styling for the Gift Info (below the gift badge) */
        .gift-info {
            margin-top: 20px;
            text-align: center;
    ...
    <section class="relative overflow-hidden bg-[#f8faf8]">
        ...
        <div class="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 sm:py-14 lg:py-16">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-[1fr_.95fr] gap-8 lg:gap-12 items-start">
                <div class="product-card rounded-[34px] border border-white/70 bg-white/85 backdrop-blur-xl">
                    <div class="zoom-container h-[360px] sm:h-[460px] lg:h-[560px] bg-[#f4f7f2]">
                        <img id="mainProductImage" src="{{ $image }}" alt="{{ $title }}"
                            class="w-full h-full object-contain">
                        @if($gift)
                            <div class="gift-badge">Gift</div>
                        @endif
                    </div>
                    ...
                    @if($gift)
                        <div class="gift-info">
                            @if($giftImage)
                                <img src="{{ asset('storage/' . $giftImage) }}" alt="Gift Image">
                            @endif
                            <p>{{ $gift }}</p>
                        </div>
                    @endif
                ...
                <div class="product-card rounded-[34px] border border-white/70 bg-white/85 backdrop-blur-xl p-6 sm:p-8 lg:p-10">
                    <span
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-6">
                        @foreach($relatedProducts as $related)
                            @php
                                ...
                                $relatedPrice = (float) ($related->sale_price ?? 0);
                            @endphp
                            <div
                                ...
                                <a href="{{ route('combo-products.show', $related) }}">
